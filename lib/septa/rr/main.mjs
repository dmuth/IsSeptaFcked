...
import { get as data_get } from "../../data.mjs";
import { set as data_set } from "../../data.mjs";
import { del as data_del } from "../../data.mjs";
...
//
import { 
	getStatusClass as text_getStatusClass,
	getMessage as text_getMessage,
	checkAge as text_checkAge,
	getLateTrainNames as text_getLateTrainNames
} from "./text.mjs";
...
*/
async function api_fetch_loop() {

	try {
		let data = await api.go();
		data = await processData(data);
		data_set("rr", "data", data);
		schedule_api_fetch_loop();
	
	} catch (e) {
		console.log("ERROR: go(): " + e.stack);
		schedule_api_fetch_loop();
	}

} // End of api_fetch_loop()
...
function schedule_api_fetch_loop() {
	...
	let timeout_sec = 60 * 1;
	//timeout_sec = 1; // Debugging - Uncomment this to timeout
	let timeout = timeout_sec * 1000;

	let message = util.format("Will fetch RR data again in %d seconds.",
		timeout_sec);
...
function age_check_loop() {
	...
	let sec = 60 * 5;
	//sec = 1; // Debugging - Uncomment this to check the data age way more often
	let ms = sec * 1000;
	...
	//
	let max_age = 60 * 10;
	//max_age = 5; // Debugging - Uncomment this to expire the data way more often

	setTimeout(async function() {

		try {

			let age = text_checkAge(data_get("rr", "data"));
			...
			console.log(`ERROR: age_check_loop(): ${error}`);

		} finally {
			console.log(`Scheduling RR data to be checked again in ${sec} sec`);
			age_check_loop();
...
function init(time, time_t) {
	...
	retval["late"] = {};
	retval["status"] = {};
	let message = text_getMessage(retval)
	retval["status"]["message"] = message["message"];
	retval["status"]["summary"] = message["summary"];
	retval["status"]["css_class"] = text_getStatusClass(retval["status"]["status"]);
...
function processData(data) {
	...
	//
	let _raw_data = data_get("rr", "raw_data");
	data_del("rr", "raw_data");
	...
	//data["late"]["10"] = getLate(data["data"], 1, 29); // Debuggng
	data["late"]["30"] = getLate(data["data"], 30, 86400);
	//data["late"]["30"] = getLate(data["data"], 1, 86400); // Debugging
	...
	if (data["num"] >= 1) {
		...
		var late = text_getLateTrainNames(data["late"]["10"]);
		for (var k in late) {
			var row = late[k];
			data["status"]["late"].push(row);
		}

		var late = text_getLateTrainNames(data["late"]["30"]);
		for (var k in late) {
			var row = late[k];
			data["status"]["late"].push(row);
		}
	...
	data["status"]["css_class"] = text_getStatusClass(data["status"]["status"]);
	let message = text_getMessage(data);
...
function prime() {
	...
	let retval = init("never", -1);

	data_set("rr", "data", retval);
