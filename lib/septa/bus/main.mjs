...
import { 
	...
	getSuspendedNames as text_getSuspendedNames
} from "./text.mjs";
...
export function getRawData() {
	...
	// caller to modify data that's returned by reference.
	//
	let data = data_get("bus", "raw_data");
	resolve(data);
...
function age_check_loop() {
	...
	//
	var sec = 60 * 10;
	//sec = 1; // Debugging - Uncomment to check age more often
	var ms = sec * 1000;
	...
	//
	var max_age = 60 * 20;
	//max_age = 5; // Debugging - Uncomment to expire much quicker

	setTimeout(function() {

		try {
			let age = text_checkAge(data_get("bus", "data"));
...
function getIsFucked(num_suspended) {
	...
	} else if (num_suspended > 5 && num_suspended <= 20) {
		retval = "fucked";

	} else if (num_suspended > 20) {
		//
		// In honor of COVID-19
		//
		retval = "fuuuuuuuucked";
...
function processData(data) {
	...
	//
	let _raw_data = data_get("bus", "raw_data");
	data_del("bus", "raw_data");
	...
	if (data["num"] >= 1) {
		...
		data["status"]["suspended"] = text_getSuspendedNames(data["suspended"]);
